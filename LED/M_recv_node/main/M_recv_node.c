...
double rssiToDist(int rssi){
    return pow(10, ((double) (-45 - rssi)) / 100);
}
        ...
        */
        ...
        if (strcmp(recv_msg.cmd, "PONG") == 0){
            ...
            */
            ...
            int rssi = recv_msg.led_level;
            blinkcount += ((double) 1.0) / rssiToDist(rssi);
            if (rssi < -50) {
                if (led == 1) {
                    off();
                    led = 0;
...
void app_main(){
    ...
    //xTaskCreatePinnedToCore(led_blink, "led", 2048, NULL, 10, NULL, 1);
    sniffer_init();
    //led_blink();
}