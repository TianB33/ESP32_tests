...
int64_t lasttime = 0, curtime;
volatile int msgcnt = 0;
...
/*timer interrupt function to see the number of packets sniffed in an interval*/
void IRAM_ATTR timer_isr(void* arg)
{
    // Print the number of packets sniffed during the previous interval
    printf("sniffed: %d\n", msgcnt);

    // Reset the packet count for the next interval
    msgcnt = 0;
}
...
void timer_init(){
    ...
    esp_timer_handle_t packet_timer;
    esp_timer_create(&timer_args, &packet_timer);

    esp_timer_start_periodic(packet_timer, 10000);
}
...
void app_main(){
    ...
    //timer_init();
    sniffer_init();
    xTaskCreatePinnedToCore(printmsgs, "print", 2048, NULL, 10, NULL, 1);
}